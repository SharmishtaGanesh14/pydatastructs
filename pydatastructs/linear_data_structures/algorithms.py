...
def radix_sort(array, **kwargs):
    """
    Implements Radix Sort algorithm and returns a new sorted array.
    ...
    >>> [sorted_arr[i] for i in range(len(sorted_arr))]
    ...
    # Create a copy of the array to avoid in-place modification
    new_array = array.clone()

    # Find the maximum number to determine the number of digits
    max_val = max(new_array[start:end + 1])

    # Perform counting sort for each digit
    exp = 1  # Exponent representing the current digit's place (ones, tens, etc.)
    
    while max_val // exp > 0:
        _counting_sort_by_digit(new_array, start, end, exp)
        exp *= 10
    
    return new_array


def _counting_sort_by_digit(array, start, end, exp):
    """
    Helper function for Radix Sort to perform counting sort on a specific digit.
    ...
    array: Array
        The array which is to be sorted.
    start: int
        Starting index of the portion to be sorted.
    end: int
        Ending index of the portion to be sorted.
    exp: int
        Current digit's place value (1 for ones place, 10 for tens place, etc.).
    ...
    count = [0] * 10  # Count array to store occurrences of each digit (0-9)
    ...
    # Build the output array using cumulative positions in count array
