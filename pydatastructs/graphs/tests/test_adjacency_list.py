...
def test_adjacency_list():
    ...
    assert [n.name for n in neighbors] == ['v_2']
    ...
    with raises(ValueError):
        g.add_edge('v', 'x')
    with raises(ValueError):
        g.remove_edge('v_1', 'v')  # Removing already removed edge
    with raises(ValueError):
        g.remove_vertex('non_existent')  # Removing a vertex that doesn't exist

    # Self-loop test
    g.add_edge('v_1', 'v_1')
    assert g.is_adjacent('v_1', 'v_1') is True
